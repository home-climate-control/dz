...
class ZoneTest {
    ...
    void enabled() {
        ...
        StepVerifier
                ...
                .assertNext(s -> {
                    ...
                    assertThat(s.getValue().thermostatStatus.calling).isTrue();
                    assertThat(s.payload).isEqualTo(name);
                })
                .assertNext(s -> assertThat(s.getValue().thermostatStatus.calling).isFalse())
                .assertNext(s -> assertThat(s.getValue().thermostatStatus.calling).isFalse())
                .verifyComplete();
    }
    ...
    void disabled() {
        ...
        StepVerifier
                ...
                .assertNext(s -> {
                    assertThat(s.getValue().settings.setpoint).isEqualTo(setpoint);
                    assertThat(s.getValue().thermostatStatus.calling).isFalse();
                    assertThat(s.payload).isEqualTo(name);
                })
                .verifyComplete();
    }
    ...
    void setpointChangeEmitsSignal() {
        ...
        // PV change
        assertThat(accumulator.get(0).getValue().thermostatStatus.calling).isFalse();
        assertThat(accumulator.get(1).getValue().thermostatStatus.calling).isTrue();

        // Setpoint change
        assertThat(accumulator.get(2).getValue().thermostatStatus.calling).isFalse();

        // PV change again
        assertThat(accumulator.get(3).getValue().thermostatStatus.calling).isTrue();
