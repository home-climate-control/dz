...
public final class GCalScheduleUpdater extends GCalScheduleUpdaterBase {
    ...
    public Map<Thermostat, SortedMap<Period, ZoneStatus>> update() throws IOException {
        ...
        var ts2schedule = new TreeMap<Thermostat, SortedMap<Period, ZoneStatus>>();
        ...
        try {
            ...
            logger.info("{} calendars found:", calendars.size());
            ...
            for (var pair : ts2schedule.entrySet()) {

                logger.info("{}: {} entries", pair.getKey().getName(), pair.getValue().size());

                for (var periodZoneStatusEntry : pair.getValue().entrySet()) {
                    logger.info("  {}", periodZoneStatusEntry);
                }
            }
    ...
    private void updateCalendar(Map<Thermostat, SortedMap<Period, ZoneStatus>> ts2schedule, Calendar calendarClient, CalendarListEntry calendar) {
        ...
        try {
            ...
            if (tSet == null || tSet.isEmpty()) {
                ...
                logger.debug("No zone '{}' configured, skipped", name);
                return;
            }
    ...
    private void parse(Map<Thermostat, SortedMap<Period, ZoneStatus>> ts2schedule, Set<Thermostat> tSet, List<Event> events) {
        ...
        var period2status = new TreeMap<Period, ZoneStatus>();

        for (Event event : events) {
            updateEvent(period2status, event);
        }

        for (Thermostat thermostat : tSet) {
            ts2schedule.put(thermostat, period2status);
        }
    }
